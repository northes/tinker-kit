...
- 规范化不得把内部 ID 写入展示字段；展示名称应回退到被引用实体的名称（例如镜像来源名回退到 `SSHProfiles` 的 `Name`，而不是 `SSHProfileID`）。
- 规范化顺序：先规范化被引用实体（`SSHProfiles`），再规范化引用方（`ImageSources`），否则取不到用于默认名称的实体。
- 读取配置时要顺带修复历史坏数据（例如把等于 `SSHProfileID` 的旧名称纠正为配置名），不要只在下次写入时处理。
